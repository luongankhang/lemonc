 ...
 *
 * <p>Parses the Token stream generated by {@link Lexer} according to the BNF grammar of the Lemon language,
 * to build an Abstract Syntax Tree (AST). Uses top-down LL(2) parsing strategy, handling
 * grammar ambiguities via {@code lookahead(1)} / {@code lookahead(2)}.</p>
 *
...
public class Parser {
	...
	/**
	 * Compare {@code token} with the token currently read by the lexer
	 * @param lexeme
	 * @throws IOException
	 */
	...
	private Ast.MainClass.MainClassSingle parseMainClass() throws IOException {
		...
		// Check if class name is consistent
		if( !className.equals(lexer.getClassName()) ){
			this.error(String.format("Class name '%s' is inconsistent with file name '%s'", className, lexer.getClassName()));
		}
		move();
		match("{");
		...
		if( look.kind == TokenKind.Id ){
			...
			if( look.kind == TokenKind.Lbracket) {
				...
				try {
					...
					error(String.format("Array size must be an integer, but got: '%s'", look.lexeme));
					return null;
				}
				if( size <= 0 ){
					error(String.format("Array size must be a positive integer, but got: %d", size));
					return null;
				}
				...
				if( arrayType == null ){
					error(String.format("Unsupported array base type: %s", type));
					return null;
				}
			...
			error("Invalid declaration format, variable name must follow type");
			return null;
		}
	}
	...
	private Ast.Declare.T parseFormalParam() throws IOException {
		...
		if (look.kind == TokenKind.Lbracket) {
			...
			if (arrayType == null) {
				error(String.format("Unsupported array parameter base type: %s", type));
				return null;
			}
			type = arrayType;
		}
	...
	private Ast.Stmt.T parseStmt() throws IOException {
		...
		if( look.kind == TokenKind.Printf ){
			...
			if (look.kind != TokenKind.String) {
				error("The first parameter of printf must be in string format");
			}
	...
	private Ast.Stmt.T parseSimpleStmtWithoutTerminator() throws IOException {
		if (look.kind != TokenKind.Id) {
			error("Expected assignment statement or method call");
		}
		Token ahead = lexer.lookahead(1);
		if( ahead.kind == TokenKind.Lparen ){
			String mthName = look.lexeme;
			int lineNumber = look.lineNumber; // S dng lineNumber
			Ast.Expr.T expr = parseMethodCall();
			if( expr instanceof Ast.Expr.Call){
				// S dng constructor c
			...
			String arrayName = look.lexeme;
			int lineNum = look.lineNumber; // S dng lineNumber
			match( new Token(TokenKind.Id) );
			match( "[" );
			Ast.Expr.T index = parseExpr();
			match( "]" );
			...
			Token idToken = look; // Lu token id
			String id = idToken.lexeme;
			int lineNum = idToken.lineNumber; // S dng lineNumber
			match( new Token(TokenKind.Id) );
			match( new Token(TokenKind.Assign) );
			Ast.Expr.T expr = parseExpr();
			// S dng constructor c cho Id v Assign
			Ast.Expr.Id idExpr = new Ast.Expr.Id(id, lineNum); // Dng lineNumber
			return new Ast.Stmt.Assign(idExpr, expr, lineNum); // Dng lineNumber
		}
		error("Failed to parse simple statement");
		return null;
	}
	...
	private Ast.Expr.T parseAdditiveExpr() throws IOException {
		...
		while(look.kind==TokenKind.Add
			...
			Token opToken = look; // Lu token ca ton t (+ hoc -)
			move();
			Ast.Expr.T otherExpr = parseTerm();
			// S dng lineNumber t token ton t, quay li dng constructor c
			if(opToken.kind==TokenKind.Add) {
				expr = new Ast.Expr.Add(expr, otherExpr, opToken.lineNumber); // Dng lineNumber
			}else {
				expr = new Ast.Expr.Sub(expr, otherExpr, opToken.lineNumber); // Dng lineNumber
			}
		}
		return expr;
	}
	...
	private Ast.Expr.T parseFactor() throws IOException{
		...
		if(look.kind==TokenKind.Lparen){
			...
			// Float literal defaults to float type (for backward compatibility)
			expr = new Ast.Expr.Number(new Ast.Type.Float(),look.lexeme,look.lineNumber);
			move();
			return expr;
		}else if( look.kind==TokenKind.Id ){
			Token temp = look; // Lu token id
			Token ahead = lexer.lookahead(1);
			if( ahead.kind == TokenKind.Lparen){
			...
			// Array access: arr[i]
				...
				String arrayName = look.lexeme;
				int lineNum = temp.lineNumber; // S dng lineNumber
				move(); // consume id
				match("[");
				Ast.Expr.T index = parseExpr();
				match("]");
				// S dng constructor c
				expr = new Ast.Expr.ArrayAccess(arrayName, index, lineNum); // Dng lineNumber
				...
				if (!"length".equals(look.lexeme)) {
					error("Array property only supports length");
				}
				match(new Token(TokenKind.Id));
				// S dng constructor c
				expr = new Ast.Expr.ArrayLength(arrayName, lineNum); // Dng lineNumber
			}
			else{
				// S dng lineNumber cho constructor c
				expr = new Ast.Expr.Id(look.lexeme, temp.lineNumber); // Dng lineNumber
				expr.setSpan(tokenSpan(temp));
				move();
			}
			return expr;
