 ...
 *
 * <p>Traverses the AST based on the Visitor pattern, performing the following static semantic checks:</p>
 * <ul>
 *   <li><b>Type checking</b>: Type consistency in assignments, operations, and method calls</li>
 *   <li><b>Variable checking</b>: Undeclared variables, uninitialized before use</li>
 *   <li><b>Method checking</b>: Undefined methods, duplicate definitions, parameter count/type matching</li>
 *   <li><b>Control flow checking</b>: if/while conditions must be bool type</li>
 *   <li><b>Return value checking</b>: Return type matches method declaration, main method must be void</li>
 * </ul>
 *
 * <p>Throws {@link SemanticException} when errors are found, with the error message format
 * {@code [Semantic Analysis] Line N: Error description}.</p>
 *
 * @author andy
...
public class SemanticVisitor implements ISemanticVisitor {
    ...
    public void visit(Ast.Expr.Id obj) {
        ...
        if( currMethodLocalVar.contains(obj.getId()))
            error(obj.getLineNum(),String.format("Variable '%s' is used before assignment",obj.getId()));
        if( obj.getType() == null ) {
            // Type might not have been successfully parsed in Parser phase (variable not registered in varTable)
    ...
    private void validateMainMethod() {
        ...
        if (main.getRetType().getKind() != TypeKind.VOID) {
            error(main.getLineNum(), "Return type of main method must be void, actually declared as: "
    ...
    public void visit(Ast.Method.MethodSingle obj) {
        ...
        if( obj.getId().equals("main")){
            if( obj.getRetType().getKind() != TypeKind.VOID)
                error(obj.getLineNum(), "Return type of main method must be void, actually declared as: " + typeName(obj.getRetType()));
        }
        ...
        if( !obj.getId().equals("main")
            ...
            error(obj.getLineNum(), "Non-void method '" + obj.getId() + "' does not have return on all paths");
        }
    }
    ...
    public void visit(Ast.Expr.Number obj) {
        if(obj.getType() instanceof Ast.Type.Int){
            ...
            error(obj.getLineNum(),"Unsupported number type: " + typeName(obj.getType()));
        }
    }
    ...
    public void visit(Ast.Stmt.Printf obj) {
        ...
        if (placeholders.size() != argCount) {
            error(obj.getLineNum(), String.format(
                    "printf parameter count mismatch: format string needs %d, actual %d",
    ...
    public void visit(Ast.Stmt.Return obj) {
        ...
        if( this.typeOfMethodDeclared != null
            ...
            error(obj.getLineNum(), "void methods cannot return a value");
        }
        this.visit(obj.getExpr());
    ...
    public void visit(Ast.Stmt.Break obj) {
        if (loopDepth <= 0)
            error(obj.getLineNum(), "break statement must be enclosed in a loop body.");
    }

    @Override
    public void visit(Ast.Stmt.Continue obj) {
        if (loopDepth <= 0)
            error(obj.getLineNum(), "continue statement must be enclosed in a loop body.");
    }
    ...
    private ArrayList<Character> printfPlaceholders(String format, int lineNum) {
        ...
        for (int i = 0; i < format.length(); i++) {
            ...
            if (i + 1 >= format.length()) {
                error(lineNum, "%% in printf format string is missing placeholder");
            }
    ...
    private Ast.Type.T validateMethodCall(String methodName, ArrayList<Ast.Expr.T> inputParams,
        ...
        if (inputParams.size() != method.getFormals().size()) {
            error(lineNum, String.format("Parameter count mismatch for method '%s': expected %d, actual %d",
