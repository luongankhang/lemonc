...
/**
 * Compiler end-to-end integration tests.
 * Each .lemon example file corresponds to an independent @Test method,
 * Verifies full compilation pipeline: Lexer -> Parser -> SemanticVisitor -> TranslatorVisitor -> ByteCodeGenerator -> Jasmin.
 */
public class CompilerTest {
    ...
    // ===== Basic calculation =====
    @Test public void testCal() throws IOException { compileAndVerify("Cal", "Factorial of 10 is 3628800"); }
    @Test public void testCal01() throws IOException { compileAndVerify("Cal01", "k2=19,n=3615\n"); }
    // Example01/03/05 class name mismatch with filename (TestMain/MulTable), known issue
    ...
    // BoolTest02 class name written as BoolTest01, known filename mismatch issue
    ...
    // If04 syntax error (if body missing left brace), known issue
    ...
    @Test public void testGauss() throws IOException {
        compileAndVerify("Gauss", "Start number is: 1, end number is: 100\nSum from 1 to 100 is 5050");
    }
    ...
    @Test public void testFib() throws IOException {
        compileAndVerify("Fib",
                "Calculate Fibonacci sequence recursively, total 144 pairs of rabbits after one year\n"
                        + "Calculate Fibonacci sequence with loop, total 144 pairs of rabbits after one year\n");
    }
    ...
    // Return.lemon class name is TestMain which does not match filename, known issue
    ...
    // ===== Multiple compilations (verify Label.resetCounter works) =====
    @Test
    ...
    public void testMultipleCompilationsLabelReset() throws IOException {
        // Compile two different files consecutively, verify label no conflict
    ...
    /**
     * Completely compile a .lemon file and verify output of each stage.
     */
    ...
    /**
     * Completely compile a .lemon file; when expected output is provided, also run generated JVM class and verify stdout.
     */
    ...
    private void assertJvmOutput(String className, File classDir, String expectedOutput) throws IOException {
        ...
        try {
            if (!process.waitFor(10, TimeUnit.SECONDS)) {
                ...
                fail("JVM execution timeout: " + className);
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            fail("Waiting for JVM execution interrupted: " + className);
        }

        String output = normalizeNewlines(readAll(process.getInputStream()));
        assertEquals("JVM exit code should be 0, output:\n" + output, 0, process.exitValue());
        assertEquals("JVM output does not match expected: " + className,
